...
func checkAge(first: String) -> Int? {
    if let age = Int(first) {
        if (age > 18) {
            print("Welcome!")
        } else {
            print("Sorry, but you arent old enough to use your app")
        }
    return age
} else {
...
func sales(purchase: String) -> Double? {
    if (stock[purchase]! > 0) {
...
sales(purchase: "Chips")
sales(purchase: "Donuts")
