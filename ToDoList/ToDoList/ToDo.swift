...
//  ToDoList
//
//  Created by Robert Steed on 11/6/22.
//
...
struct ToDo: Equatable, Codable {
    ...
    static func loadToDos() -> [ToDo]? {
        guard let codedToDos = try? Data(contentsOf: ToDoCell.archiveURL) else {return nil}
    ...
    static func saveToDos(_ toDos: [ToDo]) {
        ...
        try? codedToDos?.write(to: ToDoCell.archiveURL, options: .noFileProtection)
    }
